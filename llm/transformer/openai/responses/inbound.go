...
func convertToMessageContent(content Input, ownerTypes ...string) llm.MessageContent {
	items := convertToMessageContentParts(content, ownerTypes...)
...
func convertToMessageContentParts(input Input, ownerTypes ...string) []llm.MessageContentPart {
	ownerType := ""
	if len(ownerTypes) > 0 {
...
func convertContentItemToPart(item *Item, ownerTypes ...string) (*llm.MessageContentPart, error) {
	ownerType := ""
	if len(ownerTypes) > 0 {
		ownerType = ownerTypes[0]
	}
...
// convertToolsToLLMWithRawIDs converts tool declarations and records their raw
// input identity so additional_tools and nested namespaces can be replayed.
